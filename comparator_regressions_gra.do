clear all
global path "//DATA1/SPR/DATA/SPRLP/EM/Review of Conditionality/Data/Comparators"
...
* Label outliers
local var "ngdp_rpch ggxcnl_gdp lngdppc pcpie_pch bfd_gdp_bp6 d_gdp bca_gdp_bp6 exchange_rate iip_gdp trade_openness credit_gdp_gra quota_gdp reserve_gdp"
...
gen credit_gdp_gra_l1=l.credit_gdp_gra
...
label var credit_gdp_gra_l1 "Fund credit outstanding/GDP (t-1)"
...
// Include treated units (prg_init_gra==1) and control units (control_group_gra==1) in the regressions
eststo clear
eststo: probit prg_init_gra ngdp_rpch_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 ggxcnl_gdp_l1 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 float_er_l1 reserve_gdp_l1 credit_gdp_gra_l1 d_region* d_year* if year<=2017 & year>=2010 & (prg_init_gra==1 | control_group_gra==1)
predict pscore1 if e(sample), pr

/*
// Profit fixed effect model is biased, whereas logit fixed effect model is unbiased.
eststo: xtlogit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 reserve_gdp_l1 i.region_num  if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1), fe
eststo: xtlogit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 reserve_gdp_l1 i.region_num  if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1), fe

// ROC 2011 used population average method.
eststo: xtprobit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 reserve_gdp_l1 d_region* if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1) , pa
eststo: xtprobit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 reserve_gdp_l1 d_region* if year<=2017 & year>=2011 & (prg_init_gra==1 | control_group_gra==1) , pa
predict pscore1 if e(sample), mu
*/

esttab using "${path}/Results/propensity_gra_${date_str}.csv", b(%9.3f) se(%9.3f) t() star(* 0.10 ** 0.05 *** 0.01) drop(_cons) scalars (N) ar2 nogaps label nodepvar replace



* Propensity Score Matching
pscore prg_init_gra ngdp_rpch_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 ggxcnl_gdp_l1 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 float_er_l1 reserve_gdp_l1 credit_gdp_gra_l1 d_region* d_year* if year<=2017 & year>=2010 & (prg_init_gra==1 | control_group_gra==1), pscore(mypscore)
psmatch2 prg_init_gra, outcome(ngdp_rpch) pscore(mypscore) neighbor(100) // 100 is for selecting 5 countries from the SAME YEAR


* Export programs with comparators
sort _id
gen program = country_name + ", " + string(year) + ", " + arrangement_type if prg_init_gra==1
...
forvalues i=1(1)100 {
quietly gen diff_nbr_prg_`i' = ctry_`i' + ", " + string(year_`i') if prg_init_gra==1
quietly gen same_nbr_prg_`i' = ctry_`i' + ", " + string(year_`i') if prg_init_gra==1 & year_`i'==year
}
...
export excel program diff_prg_* diff_dis_* using "${path}/Results/comparator_gra_${date_str}.xlsx" if prg_init_gra==1 & year>=2008, sheet("comparator_gra_diff_table", modify) cell(A2) firstrow(variables)
export excel program same_prg_* same_dis_* using "${path}/Results/comparator_gra_${date_str}.xlsx" if prg_init_gra==1 & year>=2008, sheet("comparator_gra_same_table", modify) cell(A2) firstrow(variables)
...
foreach v of local var {
export excel prg_time `v'0 `v'1 using "${path}/Results/comparator_charts_${date_str}.xlsx", sheet("comparators_gra_`v'", modify) cell(A22) 
}

*xtprobit prg_init_gra l.ngdp_rpch l.loggdppc l.pcpie_pch l2.pcpie_pch l.bfd_gdp_bp6 l.chg_d l.bca_gdp_bp6 d_region* if year<=2011 & year>=2002 & PRGT==0 & g7!=1 & prg_period==0, pa
