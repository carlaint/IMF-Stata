cd "\\data1\SPR\DATA\SPRLP\EM\Program forecast error\Regressions\ROC"
...
*/
gen program_ongoing = 0
replace program_ongoing = 1 if review == "" & ((action > app1 & action < end1) | (action > app2 & action < end2) | (action > app3 & action < end3) | (action > app4 & action < end4) | (action > app5 & action < end5)) 

* JY: add the conditon of review!="" to make sure that all MONA data are part of program_country
gen program_country = 0
replace program_country = 1 if review !="" | ((action >= app1 & action <= end1) | (action >= app2 & action <= end2) | (action >= app3 & action <= end3) | (action >= app4 & action <= end4) | (action >= app5 & action <= end5))  
...
gen PastProgram_d=0
replace PastProgram_d= 1 if (action> app1)
replace PastProgram_d= 2 if (action> app2)
replace PastProgram_d= 3 if (action> app3)
replace PastProgram_d= 4 if (action> app4)
replace PastProgram_d= 5 if (action> app5)
...
* Main Specification for total sample (use this for template) this is now with WEO data only. To include MONA, switch to condition program_ongoing==0 instead of (program_ongoing == 1 | review == "") 
...
*/  if (program_ongoing == 1 | review == "") &  years_to_forecast>=0  & year <= 2016 & (month(action)<=10 | year!=year(action)) , fe 
outreg2 using main_specification.xls, replace ctitle(Fixed effects) 
...
xtreg growth_error partnergrowth_error pb_adjustment prog_pb_adjustment high_pb_adjustment  ca_adjustment prog_ca_adjustment high_ca_adjustment /*
*/ time_sb oil_notexp_error oil_exp_error commodity_notexp_error commodity_exp_error time_to_forecast GFC /*
*/ if (program_ongoing == 1 | review == "") & years_to_forecast>=0 & year <= 2016  & (month(action)<=10 | year!=year(action)) , re 
outreg2 using main_specification.xls, append ctitle(Random effects) 
...
outreg2 using main_specification.xls, append ctitle(Cochrane-Orcutt Fixed effects) 
...
*/  if (program_ongoing == 1 | review == "") & years_to_forecast>=0 & year <= 2016  & (month(action)<=10 | year!=year(action)) , endog(time_sb pb_adjustment prog_pb_adjustment high_pb_adjustment ca_adjustment prog_ca_adjustment high_ca_adjustment program_country exceptional_d) 
...
outreg2 using main_specification.xls, append ctitle(Hausman-Taylor Estimation)
...
xtreg growth_error partnergrowth_error pb_adjustment prog_pb_adjustment high_pb_adjustment ca_adjustment prog_ca_adjustment high_ca_adjustment /*
*/ time_sb oil_notexp_error oil_exp_error commodity_notexp_error commodity_exp_error time_to_forecast bankcrisis GFC /*
*/  if program_ongoing == 0 & years_to_forecast>=0 & year <= 2016 & (month(action)<=10 | year!=year(action)) , fe 
