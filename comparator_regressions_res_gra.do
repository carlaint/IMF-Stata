...
label var credit_gdp_gra "GRA credit outstanding/GDP (t-1)"
...
eststo clear
eststo: probit prg_init_gra ngdp_rpch_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 credit_gdp_gra_l1 if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1)
eststo: probit prg_init_gra ngdp_rpch_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 credit_gdp_gra_l1 if year<=2017 & year>=2011 & (prg_init_gra==1 | control_group_gra==1)
predict pscore1 if e(sample), pr

/*
// Profit fixed effect model is biased, whereas logit fixed effect model is unbiased.
eststo: xtlogit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 i.region_num  if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1), fe
eststo: xtlogit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 i.region_num  if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1), fe

// ROC 2011 used population average method.
eststo: xtprobit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 d_region* if year<=2011 & year>=2002 & (prg_init_gra==1 | control_group_gra==1) , pa
eststo: xtprobit prg_init_gra ggxcnl_gdp_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 d_region* if year<=2017 & year>=2011 & (prg_init_gra==1 | control_group_gra==1) , pa
predict pscore1 if e(sample), mu
*/

esttab using "${path}/Results/propensity.csv", b(%9.3f) se(%9.3f) t() star(* 0.10 ** 0.05 *** 0.01) drop(_cons) scalars (N) ar2 nogaps label nodepvar replace



* Propensity Score Matching
pscore prg_init_gra ngdp_rpch_l1 lngdppc_l1 pcpie_pch_l1 pcpie_pch_l2 bfd_gdp_bp6_l1 d_gdp_l1 bca_gdp_bp6_l1 exchange_rate_l1 credit_gdp_gra_l1 if year<=2017 & year>=2011 & (prg_init_gra==1 | control_group_gra==1), pscore(mypscore)
...
export excel program diff_prg_* diff_dis_* using "${path}/Results/comparator_res.xlsx" if prg_init_gra==1 & year>=2011, sheet("comparator_diff_table", modify) cell(A2) firstrow(variables)
export excel program same_prg_* same_dis_* using "${path}/Results/comparator_res.xlsx" if prg_init_gra==1 & year>=2011, sheet("comparator_same_table", modify) cell(A2) firstrow(variables)
...
foreach v of local var {
export excel prg_time `v'_orig0 `v'_orig1 using "${path}/Results/comparator_res_charts.xlsx", sheet("res_gra_`v'", modify) cell(A22) 
}
